...
The image above is Figure 2 from the article we replicated. It shows that as EPV increases, all models report higher testing accuracy and that between models accuracy became more consistent, as measured by mean absolute error and root mean squared error. Different models had consistent Brier scores given a specific EPV. Additionally as EPV increases, all models show an improvement in calibration, as measured by $\Delta$AUC, the difference in AUC between the underlying truth and the model fit on the simulated data, CIL, and CS. Ridge regression performs better than maximum likelihood for both $\Delta$ AUC and CS and performs similarly in CIL.
...
This plot is a replication of Figure 2 from the paper. This verifies the conclusions of the paper, where accuracy increases with EPV and ridge regression performs better than maximum likelihood, especially when EPV is low. This plot also reveals that random forests perform worse than both ridge regression and maximum likelihood, especially as EPV increases. This is because the underlying data was simulated with a linear relationship in all data sets. Both ridge regression model linear relationships, but random forests use model using step functions, which are ill suited to linear relationships. As EPV and thus the number of data points simulated increases, both ridge regression and ML are better able to replicate the underlying linear relationship. In contrast, random foresting demonstrates only mild improvements with the increase in data.

```{r, echo = F, out.width = "60%"}
include_graphics("../images/fig3_heatmap_article.jpg")
```

This image is figure 3 from the paper we replicated. It shows that across all metrics ridge regression performs better than maximum likelihood, examining a variety of simulation inputs.
...
ggplot(fig3) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
These plots are our replication of figure 3. This matches the conclusions drawn by the box plot. Ridge regression and ML performed as expected from the paper, while random forest performs worse than both. It is difficult to conclude from the random forest model whether EPV is a meaningful metric for non-parametric models. The random forest was unsuited to the linear data and so consistently performed with lower accuracy than either of the other two models, regardless of EPV. The random forest did show similar trends to the other two models, so it is possible that EPV is likewise incapable of encapsulating all the nuances regarding accuracy estimation, but further study including the simulation of non-linear relationships between predictors and the outcome would be required to verify this.
...
Our results closely align with these conclusions. Across the three models, predictive performance generally improved with increasing EPV. Ridge regression consistently demonstrated stable behavior in low EPV settings and adapted naturally as complexity increased, while maximum likelihood performance improved as EPV grew. Random forest models likewise demonstrated improved performance as EPV grew.
