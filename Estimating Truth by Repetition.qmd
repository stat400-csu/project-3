...
-   different combinations of EPV, event ratios, AUC, and #, distribution, effect of predictors
-   7 different models: Maximum Likelihood, backwards subset selection, Firth's penalized likelihood, heuristic shrinkage, Ridge, LASSO
-   6 predictive performance metrics: rMPSE, MAPE, calibration slope, average calibration in large, $\Delta$AUC, Brier
...
-   EPV (5, 10, 50), number of predictors (P) (4, 8, 12), distribution of predictors (MVN(0), MVN(0.5)), beta patterns: (equal effect, 1 strong, 1 noise, 1/2 noise)
...
::: smallest
Discrimination improves with sample size and is strongest under predictor correlation (ρ = 0.5)  
:::
:::
::::
...
## Random Forest
process: fit 500 trees and at each split, consider $\sqrt{P}$ randomly selected predictors
...
```
on average, higher EPV improves accuracy
...
::: {.column width="40%"}
```{r, fig.width=4, fig.height=6, fig.margin = 0}
