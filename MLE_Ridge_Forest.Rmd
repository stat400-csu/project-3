...
```{r}
fig3 <- combined_metrics |> pivot_longer(cols = c(rMPSE, MAPE, Brier), names_to = "measure",
                                         values_to = "value") |>
  mutate(measure = factor(measure, level = c("rMPSE", "MAPE", "Brier")),
         model = factor(model, level = c("Ridge", "MLE", "Random Forest")))
levels(fig3$model) <- c("Ridge", "ML", "Random Forest")
str(fig3)
ggplot(fig3) + 
